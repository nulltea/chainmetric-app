...
class AssetsSearchDelegate extends SearchDelegate<int> {
  ...
  Widget buildResults(BuildContext context) => FutureBuilder<AssetsResponse?>(
        future: AssetsController.getAssets(
            query: AssetsQuery()..limit = _itemsLength),
        builder: (context, snapshot) => snapshot.data != null
            ? ListView.builder(
                itemCount: snapshot.data!.items.length,
                padding: const EdgeInsets.symmetric(vertical: 12),
                itemBuilder: _listBuilder(snapshot.data!.items) as Widget
                    Function(BuildContext, int))
            : Container(),
      );

  Function(BuildContext context, int index) _listBuilder(
